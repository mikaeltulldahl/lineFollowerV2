...
//
//#define UART_TX_DATA_ITEMS 3
//#define UART_TX_BUFFERSIZE (4+4*UART_TX_DATA_ITEMS)
//volatile uint8_t uart_tx_buf[UART_TX_BUFFERSIZE];
//
//volatile bool uart_tx_complete = true;
//volatile bool uart_send_which_data[UART_TX_DATA_ITEMS]; // stores which data to send to processing,
//void sendInt(int r) {
//
//	if(r == 0)
//		sendByte('0');
...
//	USART_InitTypeDef USART_InitStructure;
//
//	USART_InitStructure.USART_BaudRate = 9600;//230400;//9600;	//115200;
//	USART_InitStructure.USART_WordLength = USART_WordLength_8b;
//	USART_InitStructure.USART_StopBits = USART_StopBits_1;
//	USART_InitStructure.USART_Parity = USART_Parity_No;
//	USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;
...
//	DMA_InitStructure.DMA_Memory0BaseAddr =(uint32_t)uart_tx_buf; // tog bort & framför
//	DMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToPeripheral;
//	DMA_InitStructure.DMA_BufferSize = (uint16_t)UART_TX_BUFFERSIZE;
//	DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
//	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
//	DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;
//	DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
...
//
////	int i;
////	for(i=0; i < UART_TX_BUFFERSIZE ;i++){
////		uart_send_which_data[i] = false;
////	}
//
//	USART_DMACmd(USART1,USART_DMAReq_Tx, ENABLE);
//
...
//
//
////fungerar inte, blir ej kallad på
//void DMA2_Stream7_IRQHandler(void){
//	//TODO kolla om TC enabled, dock onödigt nu pga inga andra interrupt aktiverade
//	//clear pending bit
//	DMA_ClearITPendingBit(DMA2_Stream7,DMA_IT_TCIF7);
...
//		case 'P' :
//			while(USART_GetFlagStatus(USART1, USART_FLAG_RXNE) == RESET); /* Wait for next rx */
//			Ggyro_p = 0.01f*0.00392156862f* USART1->DR; // the following value is the parameter
//			//		    	Kp_p = 0.00392156862f* USART1->DR; // the following value is the parameter
//			//if 255 is sent, kp will end up as around 1
//			break; /* optional */
//		case 'I' :
//			while(USART_GetFlagStatus(USART1, USART_FLAG_RXNE) == RESET); /* Wait for next rx */
//			Ggyro_r = 0.01f*0.00392156862f* USART1->DR; // the following value is the parameter
//			//		    	Ki_p = 0.00392156862f* USART1->DR; // the following value is the parameter
//			break; /* optional */
//		case 'D' :
//			while(USART_GetFlagStatus(USART1, USART_FLAG_RXNE) == RESET); /* Wait for next rx */
//			Ggyro_y = 0.01f*0.00392156862f* USART1->DR; // the following value is the parameter
//			//		    	Kd_p = 0.00392156862f* USART1->DR; // the following value is the parameter
//			break; /* optional */
//		case 'C' :
//			while(USART_GetFlagStatus(USART1, USART_FLAG_RXNE) == RESET); /* Wait for next rx */
...
//	}
//
////	int d = DMA_GetITStatus(DMA2_Stream7, DMA_IT_TCIF7);
//	DMA_ITConfig(DMA2_Stream7,DMA_IT_TC, DISABLE);
//	// wait on previous transmit
////	while(!uart_tx_complete);
//
//	//fill buffer with data
//	// start wich the 4 sync-bytes
//	uart_tx_buf[0] = 0xFF;
//	uart_tx_buf[1] = 0xFF;
//	uart_tx_buf[2] = 0x00;
//	uart_tx_buf[3] = 0xFF;
...
//	//int i;
//	int j = 4;
//	//	for (i = 0; i < UART_TX_DATA_ITEMS; ++i) {
//	//		//TODO check which items to sent, now we send all of them
//	//		//if(uart_send_which_data[i]){ // check if this data should be transmitted
//	//		uart_bufferFloat(uart_tx_buf + j, 1.1);
//	//		j=j+4;
//	//
//	//
//	//	}
//
//	uart_bufferFloat(uart_tx_buf + j, accelerometer_data[0]);
//	j=j+4;
//	uart_bufferFloat(uart_tx_buf + j, accelerometer_data[1]);
//	j=j+4;
//	uart_bufferFloat(uart_tx_buf + j, accelerometer_data[2]);
//	j=j+4;
////	int i;
////	for (i = 0; i < 16; ++i) {
////		uart_tx_buf[i]=i;
...
//	//		sendByte(0x00);
//	//		sendByte(0xFF);
//	//		sendFloat(accelerometer_data[0]);
//	//		sendFloat(accelerometer_data[1]);
//	//		sendFloat(accelerometer_data[2]);
//
//	//set up DMA to transmit buffer
...
////	while(DMA_GetFlagStatus(DMA2_Stream7,DMA_FLAG_TCIF7) == RESET)
//	DMA_ClearFlag(DMA2_Stream7,DMA_FLAG_TCIF7);
////	int a = DMA_GetCurrDataCounter(DMA2_Stream7);
//	//DMA_Cmd(DMA2_Stream7,DISABLE);
////	for(i = 0;i<20;i++)
////	usart_tx_buf[i] = i+51;
////	DMA_Cmd(DMA2_Stream7,ENABLE);
...
//
//	DMA_Cmd(DMA2_Stream7, DISABLE);
//	DMA_SetCurrDataCounter(DMA2_Stream7, UART_TX_BUFFERSIZE);
//
//	/* Reset DMA Stream registers (for debug purpose) */
//	DMA_DeInit(DMA2_Stream7);
//	DMA_InitStructure.DMA_Memory0BaseAddr =(uint32_t)uart_tx_buf; // tog bort & framför
...
//	while ((DMA_GetCmdStatus(DMA2_Stream7) != ENABLE)){
//	}
////	a = DMA_GetCurrDataCounter(DMA2_Stream7);
////	delay_us(50);
////	a = DMA_GetCurrDataCounter(DMA2_Stream7);
////	int c = 1;
//	DMA_ITConfig(DMA2_Stream7,DMA_IT_TC, ENABLE); // enable the transfer complete interrupt
